...
| `pages` | After release / manual | Deploy documentation via GitHub Pages |
...
2. Tags the container image at `ghcr.io/stuttgart-things/provider-clusterbook:<version>`
3. Builds and pushes the Crossplane xpkg to `ghcr.io/stuttgart-things/provider-clusterbook-xpkg:<version>`
...
# Docker build
docker build -f cluster/images/provider-clusterbook/Dockerfile -t provider-clusterbook:test .
```
